    ...
    U: Clone + Copy + From<f32>,
{
    ...
    pub fn evaluate(&self, state: &S, action: &A) -> T {
        self.features
            ...
            .map(|(f, &w)| f.evaluate(state, action) * w)
            .sum::<T>()
    }
}
...
mod tests {
    ...
    fn test_deep_sea_feature() {
        let oxygen_feature: Feature<'_, DeepSea, DeepSeaAction, f32> =
            Feature::new(Box::new(|s: &DeepSea, _a: &DeepSeaAction| {
                s.oxygen() as f32
            }));
        let distance_feature: Feature<'_, DeepSea, DeepSeaAction, f32> =
            Feature::new(Box::new(|s: &DeepSea, _a: &DeepSeaAction| {
                s.player_idx() as f32
            }));
        ...
        expect_eq!(
            DeepSea::OXYGEN as f32,
            oxygen_feature.evaluate(&deep_sea, &deep_sea_action)
        );
